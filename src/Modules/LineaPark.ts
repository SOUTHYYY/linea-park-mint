...
export const LineaParkActionAndStrategy: Readonly<ContractData[]> = [
  ...
  {
    address: '0x34be5b8c30ee4fde069dc878989686abe9884470',
    questUrl: 'https://layer3.xyz/quests/linea-sidus',
    questName: 'Claim Nidum Mystery Box 2',
    abi: [
      {
        inputs: [
          { internalType: 'string', name: 'name_', type: 'string' },
    ...
    params: async () => {
      ...
      const instance = axios.create({
        baseURL: 'https://auth.sidusheroes.com/api/v1',
        headers: {
      ...
      const { signature } = w3.eth.accounts.sign(
        `Please sign this message to connect to sidusheroes.com: ${nonce}`,
        PrivateKey,
      );
      ...
      await instance
        .post(
          `https://plsrv.sidusheroes.com/shadow-game-linea/api/v1/item`,
          {
      ...
      const contractData = await instance.post(
        'https://plsrv.sidusheroes.com/shadow-game-linea/api/v1/claim',
        body,
        { headers: { Authorization: `Bearer ${accessToken}` } },
      );
      console.log(contractData);

      const { message: _msgForSign, signature: _signature } = contractData.data;

      return [[9], [1], 0, _msgForSign, _signature];
    },
  },
];
